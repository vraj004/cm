...
MODULE REACTION_DIFFUSION_FINITE_ELASTIC_PROBLEM_ROUTINES
  ...
  PUBLIC REACDIFF_FINELASTIC_CONTROL_LOOP_POST_LOOP
  
  PUBLIC REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP

  PUBLIC REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA
...
999 CALL ERRORS("REACTION_DIFFUSION_FINITE_ELASTIC_PRE_SOLVE",ERR,ERROR)
  ...
  SUBROUTINE REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP(CONTROL_LOOP,ERR,ERROR,*)
    ...
    CALL ENTERS("REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP",ERR,ERROR,*999)
    ...
    CALL EXITS("REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP")
    RETURN
999 CALL ERRORS("REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP",ERR,ERROR)
    CALL EXITS("REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP")
    RETURN 1
  END SUBROUTINE REACDIFF_FINELASTIC_CONTROL_LOOP_PRE_LOOP

  !
  !================================================================================================================================
  !

  !>Sets up the reaction diffusion mechanics problem post loop.
  SUBROUTINE REACDIFF_FINELASTIC_CONTROL_LOOP_POST_LOOP(CONTROL_LOOP,ERR,ERROR,*)
    ...
    CALL ENTERS("REACTION_DIFFUSION_FINITE_ELASTIC_CONTROL_LOOP_POST_LOOP",ERR,ERROR,*999)
      ...
      IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN 
        SELECT CASE(CONTROL_LOOP%PROBLEM%SUBTYPE)
          CASE(PROBLEM_STRANGSPLIT_REACDIFF_FINITE_ELASTIC_SUBTYPE)
            ...
            CALL REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA(CONTROL_LOOP,ERR,ERROR,*999)
          CASE DEFAULT
            LOCAL_ERROR="Problem subtype "//TRIM(NUMBER_TO_VSTRING(CONTROL_LOOP%PROBLEM%SUBTYPE,"*",ERR,ERROR))// &
    ...
    CALL EXITS("REACDIFF_FINELASTIC_CONTROL_LOOP_POST_LOOP")
    RETURN
999 CALL ERRORS("REACDIFF_FINELASTIC_CONTROL_LOOP_POST_LOOP",ERR,ERROR)
    CALL EXITS("REACDIFF_FINELASTIC_CONTROL_LOOP_POST_LOOP")
    RETURN 1
  END SUBROUTINE REACDIFF_FINELASTIC_CONTROL_LOOP_POST_LOOP
  !
  !================================================================================================================================
  !

  !>Sets up the reaction diffusion mechanics problem post loop output data.
  SUBROUTINE REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA(CONTROL_LOOP,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    TYPE(CONTROL_LOOP_TYPE), POINTER :: MECH_CONTROL_LOOP
    TYPE(SOLVERS_TYPE), POINTER :: MECH_SOLVERS
    TYPE(SOLVER_TYPE), POINTER :: MECH_SOLVER
    TYPE(SOLVER_EQUATIONS_TYPE), POINTER :: MECH_SOLVER_EQUATIONS
    TYPE(SOLVER_MAPPING_TYPE), POINTER :: MECH_SOLVER_MAPPING
    TYPE(CONTROL_LOOP_TYPE), POINTER :: REACDIFF_CONTROL_LOOP
    TYPE(SOLVERS_TYPE), POINTER :: REACDIFF_SOLVERS
    TYPE(SOLVER_TYPE), POINTER :: REACDIFF_SOLVER
    TYPE(SOLVER_EQUATIONS_TYPE), POINTER :: REACDIFF_SOLVER_EQUATIONS
    TYPE(SOLVER_MAPPING_TYPE), POINTER :: REACDIFF_SOLVER_MAPPING
    TYPE(PROBLEM_TYPE), POINTER :: PROBLEM

    TYPE(EQUATIONS_SET_TYPE), POINTER :: EQUATIONS_SET !<A pointer to the equations set
    REAL(DP) :: CURRENT_TIME,TIME_INCREMENT
    INTEGER(INTG) :: EQUATIONS_SET_IDX,CURRENT_LOOP_ITERATION,OUTPUT_ITERATION_NUMBER
    CHARACTER(30) :: FILE,FNAME
    CHARACTER(30) :: OUTPUT_FILE


    CALL ENTERS("REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(CONTROL_LOOP)) THEN
      ...
      IF(ASSOCIATED(PROBLEM)) THEN 
        SELECT CASE(PROBLEM%SUBTYPE)
          CASE(PROBLEM_STRANGSPLIT_REACDIFF_FINITE_ELASTIC_SUBTYPE)
            !OUTPUT REACTION DIFFUSION AND FINITE ELASITICTY EQUATION SOLUTIONS AT EACH TIME STEP
            WRITE(*,*) CONTROL_LOOP%LOOP_TYPE
            SELECT CASE(CONTROL_LOOP%LOOP_TYPE)
            ...
            CASE(PROBLEM_CONTROL_TIME_LOOP_TYPE)
              ...
              CALL CONTROL_LOOP_GET(CONTROL_LOOP,[1,CONTROL_LOOP_NODE],REACDIFF_CONTROL_LOOP,ERR,ERROR,*999)
              CALL CONTROL_LOOP_SOLVERS_GET(REACDIFF_CONTROL_LOOP,REACDIFF_SOLVERS,ERR,ERROR,*999)
              CALL SOLVERS_SOLVER_GET(REACDIFF_SOLVERS,2,REACDIFF_SOLVER,ERR,ERROR,*999)
              REACDIFF_SOLVER_EQUATIONS=>REACDIFF_SOLVER%SOLVER_EQUATIONS
              IF(ASSOCIATED(REACDIFF_SOLVER_EQUATIONS)) THEN
                REACDIFF_SOLVER_MAPPING=>REACDIFF_SOLVER_EQUATIONS%SOLVER_MAPPING
                IF(ASSOCIATED(REACDIFF_SOLVER_MAPPING)) THEN
                  !Make sure the equations sets are up to date
                  DO equations_set_idx=1,REACDIFF_SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS
                    EQUATIONS_SET=>REACDIFF_SOLVER_MAPPING%EQUATIONS_SETS(equations_set_idx)%PTR
                    ...
                    IF(OUTPUT_ITERATION_NUMBER/=0) THEN
                      IF(CONTROL_LOOP%TIME_LOOP%CURRENT_TIME<=CONTROL_LOOP%TIME_LOOP%STOP_TIME) THEN
                        IF(REACDIFF_SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS.EQ.1) THEN

                          IF(CURRENT_LOOP_ITERATION<10) THEN
                            WRITE(OUTPUT_FILE,'("REACDIFF_TIME_STEP_SPEC_1_000",I0)') CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<100) THEN
                            WRITE(OUTPUT_FILE,'("REACDIFF_TIME_STEP_SPEC_1_00",I0)') CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<1000) THEN
                            WRITE(OUTPUT_FILE,'("REACDIFF_TIME_STEP_SPEC_1_0",I0)') CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<10000) THEN
                            WRITE(OUTPUT_FILE,'("REACDIFF_TIME_STEP_SPEC_1_",I0)') CURRENT_LOOP_ITERATION
                          END IF
                        ELSE
                          IF(CURRENT_LOOP_ITERATION<10) THEN
                            WRITE(FNAME, '(A15,I0,A4)') "REACDIFF_TIME_STEP_SPEC_",equations_set_idx,"_000"
                            WRITE(OUTPUT_FILE,'(A20,I0)') FNAME,CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<100) THEN
                            WRITE(FNAME, '(A15,I0,A3)') "REACDIFF_TIME_STEP_SPEC_",equations_set_idx,"_00"
                            WRITE(OUTPUT_FILE,'(A19,I0)') FNAME,CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<1000) THEN
                            WRITE(FNAME, '(A15,I0,A2)') "REACDIFF_TIME_STEP_SPEC_",equations_set_idx,"_0"
                            WRITE(OUTPUT_FILE,'(A18,I0)') FNAME,CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<10000) THEN
                            WRITE(FNAME, '(A15,I0,A1)') "REACDIFF_TIME_STEP_SPEC_",equations_set_idx,"_"
...
!                         ENDIF 
                  ...
                  ENDDO
                ELSE
                  CALL FLAG_ERROR("Solver Mappings not associated.",ERR,ERROR,*999)
                ENDIF
              ELSE
              ...
              CALL CONTROL_LOOP_GET(CONTROL_LOOP,[2,CONTROL_LOOP_NODE],MECH_CONTROL_LOOP,ERR,ERROR,*999)
              CALL CONTROL_LOOP_SOLVERS_GET(MECH_CONTROL_LOOP,MECH_SOLVERS,ERR,ERROR,*999)
              CALL SOLVERS_SOLVER_GET(MECH_SOLVERS,1,MECH_SOLVER,ERR,ERROR,*999)
              MECH_SOLVER_EQUATIONS=>MECH_SOLVER%SOLVER_EQUATIONS
              IF(ASSOCIATED(MECH_SOLVER_EQUATIONS)) THEN
                MECH_SOLVER_MAPPING=>MECH_SOLVER_EQUATIONS%SOLVER_MAPPING
                IF(ASSOCIATED(MECH_SOLVER_MAPPING)) THEN
                  !Make sure the equations sets are up to date
                  DO equations_set_idx=1,MECH_SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS
                    EQUATIONS_SET=>MECH_SOLVER_MAPPING%EQUATIONS_SETS(equations_set_idx)%PTR
                    ...
                    IF(OUTPUT_ITERATION_NUMBER/=0) THEN
                      IF(CONTROL_LOOP%TIME_LOOP%CURRENT_TIME<=CONTROL_LOOP%TIME_LOOP%STOP_TIME) THEN
                        IF(MECH_SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS.EQ.1) THEN

                          IF(CURRENT_LOOP_ITERATION<10) THEN
                            WRITE(OUTPUT_FILE,'("MECHEQUA_TIME_STEP_SPEC_1_000",I0)') CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<100) THEN
                            WRITE(OUTPUT_FILE,'("MECHEQUA_TIME_STEP_SPEC_1_00",I0)') CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<1000) THEN
                            WRITE(OUTPUT_FILE,'("MECHEQUA_TIME_STEP_SPEC_1_0",I0)') CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<10000) THEN
                            WRITE(OUTPUT_FILE,'("MECHEQUA_TIME_STEP_SPEC_1_",I0)') CURRENT_LOOP_ITERATION
                          END IF
                        ELSE
                          IF(CURRENT_LOOP_ITERATION<10) THEN
                            WRITE(FNAME, '(A15,I0,A4)') "MECHEQUA_TIME_STEP_SPEC_",equations_set_idx,"_000"
                            WRITE(OUTPUT_FILE,'(A20,I0)') FNAME,CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<100) THEN
                            WRITE(FNAME, '(A15,I0,A3)') "MECHEQUA_TIME_STEP_SPEC_",equations_set_idx,"_00"
                            WRITE(OUTPUT_FILE,'(A19,I0)') FNAME,CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<1000) THEN
                            WRITE(FNAME, '(A15,I0,A2)') "MECHEQUA_TIME_STEP_SPEC_",equations_set_idx,"_0"
                            WRITE(OUTPUT_FILE,'(A18,I0)') FNAME,CURRENT_LOOP_ITERATION
                          ELSE IF(CURRENT_LOOP_ITERATION<10000) THEN
                            WRITE(FNAME, '(A15,I0,A1)') "MECHEQUA_TIME_STEP_SPEC_",equations_set_idx,"_"
                            WRITE(OUTPUT_FILE,'(A17,I0)') FNAME,CURRENT_LOOP_ITERATION
                          END IF
                        ENDIF
                          FILE=TRIM(OUTPUT_FILE)
...
!                         ENDIF 
            ...
            CASE DEFAULT
              LOCAL_ERROR="Control loop type "//TRIM(NUMBER_TO_VSTRING(CONTROL_LOOP%PROBLEM%SUBTYPE,"*",ERR,ERROR))// &
                & " is not valid for a reaction diffusion mechanics type of a multiphysics problem class."
              CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT
          ...
          CASE DEFAULT
            LOCAL_ERROR="Problem subtype "//TRIM(NUMBER_TO_VSTRING(CONTROL_LOOP%PROBLEM%SUBTYPE,"*",ERR,ERROR))// &
              & " is not valid for a multi-physics type of field problem class."
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
        END SELECT
      ELSE
        CALL FLAG_ERROR("Problem is not associated.",ERR,ERROR,*999)
    ...
    CALL EXITS("REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA")
    RETURN
999 CALL ERRORS("REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA",ERR,ERROR)
    CALL EXITS("REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA")
    RETURN 1
  END SUBROUTINE REACDIFF_FINELASTIC_POST_LOOP_OUTPUT_DATA
