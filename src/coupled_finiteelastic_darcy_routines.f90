...
999 CALL ERRORS("COUPLED_ELASTICDARCY_GAUSS_DEFORMATION_GRADIENT_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE COUPLED_ELASTICDARCY_GAUSS_KIRCHOFF_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
      & MATERIALS_INTERPOLATED_POINT,INDEPENDENT_INTERPOLATED_POINT, &
      & KIRCHOFF_TENSOR,Jznu,DZDNU,DIMEN,ELEMENT_NUMBER,GAUSS_POINT_NUMBER,ERR,ERROR,*)
    ...
    REAL(DP), INTENT(OUT) :: KIRCHOFF_TENSOR(:,:)
    ...
    REAL(DP) :: AZL(3,3),AZU(3,3),DZDNUT(3,3),PIOLA_TENSOR_EFF(3,3),E(3,3),P,IDENTITY(3,3),AZLT(3,3),AZUT(3,3),KIRCHOFF_TENSOR_EFF(3,3)
    ...
    CALL ENTERS("COUPLED_ELASTICDARCY_GAUSS_KIRCHOFF_TENSOR",ERR,ERROR,*999)
    ...
    !PIOLA_TENSOR_EFF is the second Piola-Kirchoff tensor (PK2 or S)
    ...
    CASE(EQUATIONS_SET_ELASTICDARCY_ACTIVECONTRACTION_SUBTYPE)
      ...
      PIOLA_TENSOR_EFF(1,3)=mu*IDENTITY(1,3)+(lambda*LOG(Jznu)-mu)*AZU(1,3)
      PIOLA_TENSOR_EFF(2,3)=mu*IDENTITY(2,3)+(lambda*LOG(Jznu)-mu)*AZU(2,3)
      PIOLA_TENSOR_EFF(3,1)=PIOLA_TENSOR_EFF(1,3)
      PIOLA_TENSOR_EFF(3,2)=PIOLA_TENSOR_EFF(2,3)
      PIOLA_TENSOR_EFF(3,3)=mu*IDENTITY(3,3)+(lambda*LOG(Jznu)-mu)*AZU(3,3)
      PIOLA_TENSOR_EFF(1,1)=mu*IDENTITY(1,1)+(lambda*LOG(Jznu)-mu)*AZU(1,1)
      PIOLA_TENSOR_EFF(1,2)=mu*IDENTITY(1,2)+(lambda*LOG(Jznu)-mu)*AZU(1,2)
      PIOLA_TENSOR_EFF(2,1)=PIOLA_TENSOR_EFF(1,2)
      PIOLA_TENSOR_EFF(2,2)=mu*IDENTITY(2,2)+(lambda*LOG(Jznu)-mu)*AZU(2,2)
      ...
      PIOLA_TENSOR_EFF(1,1)=PIOLA_TENSOR_EFF(1,1)+ACTIVE_STRESS_11
      PIOLA_TENSOR_EFF(2,2)=PIOLA_TENSOR_EFF(2,2)+ACTIVE_STRESS_22
      PIOLA_TENSOR_EFF(3,3)=PIOLA_TENSOR_EFF(3,3)+ACTIVE_STRESS_33 
    ...
    IF(DIAGNOSTICS1) THEN
      ...
      CALL WRITE_STRING_MATRIX(DIAGNOSTIC_OUTPUT_TYPE,1,1,3,1,1,3, &
        & 3,3,PIOLA_TENSOR_EFF,WRITE_STRING_MATRIX_NAME_AND_INDICES,'("    PIOLA_TENSOR_EFF','(",I1,",:)',' :",3(X,E13.6))', &
        & '(17X,3(X,E13.6))',ERR,ERROR,*999)
      CALL WRITE_STRING_MATRIX(DIAGNOSTIC_OUTPUT_TYPE,1,1,3,1,1,3, &
        & 3,3,KIRCHOFF_TENSOR,WRITE_STRING_MATRIX_NAME_AND_INDICES,'("    KIRCHOFF_TENSOR','(",I1,",:)',' :",3(X,E13.6))', &
        & '(17X,3(X,E13.6))',ERR,ERROR,*999)
    ENDIF
    NULLIFY(C)

    CALL EXITS("FINITE_ELASTICITY_GAUSS_KIRCHOFF_TENSOR")
    RETURN
999 CALL ERRORS("COUPLED_ELASTICDARCY_GAUSS_KIRCHOFF_TENSOR",ERR,ERROR)
    CALL EXITS("COUPLED_ELASTICDARCY_GAUSS_KIRCHOFF_TENSOR")
    RETURN 1
  END SUBROUTINE COUPLED_ELASTICDARCY_GAUSS_KIRCHOFF_TENSOR
  !
  !================================================================================================================================
  !
